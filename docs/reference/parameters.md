...
| `TEXT<1-4096>` | `text` | `remainder` | Непустой остаток строки | Исходный `str` |
...
- `TEXT<1-4096>` на уровне `RemainderReader` считывает весь переданный ему
  остаток, включая внутренние и завершающие пробелы. Публичный
  `CommandLineParser` перед matching удаляет завершающий whitespace всей
  команды. Ограничение верхнего уровня, по которому этот шаблон принимает
  только CLI-строки, начинающиеся с `!`, реализовано вне parameter validator.
...
Validator сам не проверяет начальный `!`; это правило command parser’а для
standalone `TEXT<1-4096>`.
...
вызова; `TEXT` получает `RemainderReader`.
