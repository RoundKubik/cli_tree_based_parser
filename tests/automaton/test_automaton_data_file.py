...
from vrp_parser_automaton import CommandLineParser, ErrorCode, ErrorLine, ParsedCommand

DATA = Path(__file__).parents[2] / "data" / "commands.json"
...
def test_bundled_commands_are_unique_supported_and_compilable() -> None:
    ...
    ip_commands = [item for item in commands if "X.X.X.X" in item or "X:X::X:X" in item]
...
def test_bundled_text_routes_distinguish_comments_commands_and_unknowns() -> None:
    ...
    typo = parser.parse("disply interface brief")
    ...
    assert isinstance(typo, ErrorLine)
    assert len(typo.error.suggestions) == 5
    assert typo.error.suggestions[0].startswith("display interface")
