...
def test_all_four_group_modes_have_distinct_runtime_cardinality(
    ...
    assert all(isinstance(parser.parse(line), ParsedCommand) for line in accepted)
    assert all(isinstance(parser.parse(line), ErrorLine) for line in rejected)
...
def test_nested_choice_is_matched_inside_symbolic_required_set() -> None:
    pattern = "policy { { color { red | blue } } | metric INTEGER<1-10> } *"
    result = _parsed(
        CommandLineParser({"commands": [pattern]}),
        "policy metric 5 color blue",
    )

    assert result.primary_match.variation == ("policy metric INTEGER<1-10> color blue")
    assert [(value.raw, value.normalized) for value in result.parameters] == [("5", 5)]
