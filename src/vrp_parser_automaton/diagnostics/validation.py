...
from vrp_parser_automaton.runtime.state import Candidate, RejectedParameter
    ...
    def create(
        ...
        *,
        ...
        ordered = sorted(
            candidates,
            key=lambda item: (
                automaton.patterns[item.pattern_index].index,
                item.pattern_index,
            ),
        )
        ...
        for candidate in ordered:
            source = automaton.patterns[candidate.pattern_index]
            patterns.append(source.original)
        ...
        return ParseError(
            ...
            position=(unique_failures[0].span.start if unique_failures else None),
